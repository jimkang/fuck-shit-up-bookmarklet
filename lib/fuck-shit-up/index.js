...
function createFuckShitUp(opts) {
  ...
  return function fuckShitUp(sentence, done) {
    var pieces = _compact(sentence.split(/[\s]/g));
    var q = queue(4);
...
function buildParallelSentence(probable, pieces, posReports) {
  ...
  for (var i = 0; i < pieces.length; ++i) {
    ...
    var needToCapitalize = isCapitalized(piece);
    var startsSentence = probablyStartOfSentence(pieces, i);

    if ((!prefixedLastIteration || prefixTwoInARow) &&
      (i !== 0 || !skipTheFirstOpportunity) &&
      probable.roll(8) < 3 && // 3/8 chance
      ...
      if (!repeatsThePreviousPiece(modifier, newPieces) && 
        ...
        if (hasNoLowerCase(piece)) {
          ...
          // Capitalize the modifier.
          modifier = titleCaseWord(modifier);
          if (startsSentence) {
            // Lower case the original piece if it was a sentence-starter.
...
function shouldPrefix(posReport, piece) {
  function partOfSpeechIsGoodInReport(pos) {
    return posReport[pos] && !_contains(badBets, posReport[pos][0]);
  }
...
function probablyStartOfSentence(pieces, index) {
  ...
  if (pieces && pieces.length > 1) {
    ...
    if (previousPiece && previousPiece.length > 1) {
      if (_contains(endPunctuation, previousPiece[previousPiece.length - 1])) {
        probablyStart = true;
      }
    }
  }
